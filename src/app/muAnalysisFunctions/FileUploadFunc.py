    ...
    def remove_mus_by_range(self, input_text):
        ...
        for part in parts:
            ...
            if len(sub_parts) == 1: # Single MU
                ...
                if mu_idx < 0: raise ValueError("Indices must be positive.")
                ...
                if mu_start_idx < 0 or mu_end_idx < 0: raise ValueError("Indices must be positive.")
        ...
        emgfile["BINARY_MUS_FIRING"] = emgfile["BINARY_MUS_FIRING"].iloc[:, indices_to_keep]
        ...
        if not emgfile["ACCURACY"].empty:
            emgfile["ACCURACY"] = emgfile["ACCURACY"].iloc[indices_to_keep].reset_index(drop=True)
