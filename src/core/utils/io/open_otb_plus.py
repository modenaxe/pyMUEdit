...
def open_otb_plus(inputfile: str, import_window: "ImportDataWindow | None" = None) -> dict[str, Any]:
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        logger.debug("Extracting OTB archive...")
        ...
        logger.debug(f"Found {len(sig_files)} signal files: {sig_files}")
        ...
        logger.debug(f"Using signal file: {trial_label_sig}")
        logger.debug(f"Using XML file: {trial_label_xml}")

        # read the contents of the trial xml file
        logger.debug("Parsing XML configuration...")
        ...
        logger.debug(f"Parsed XML: fsamp={fsamp}Hz, nADbit={nADbit}, nchans={nchans}")

        # read in the EMG trial data
        logger.debug("Reading EMG data...")
        ...
        logger.debug(f"EMG data shape: {emg_data.shape}")

        # convert the data from bits to microvolts
        logger.debug("Converting data from bits to microvolts...")
        ...
        logger.debug(f"Found {len(sip_files)} SIP files: {sip_files}")
        if len(sip_files) != 0:
            logger.debug("Reference signals exist, loading target and path data...")
